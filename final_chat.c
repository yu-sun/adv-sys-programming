...
launch_server(void)
{
    ...
    socklen_t adr_size = sizeof(server_adr);
    int str_len, i, ret, count, j = 1;
    ...
    setsockopt(serverSock, SOL_SOCKET, SO_REUSEADDR, (void *)&j, sizeof(j));

    server_adr.sin_family = AF_INET;
    server_adr.sin_addr.s_addr = INADDR_ANY;
    server_adr.sin_port = htons(PORT);
    if ((ret = bind(serverSock, (struct sockaddr *)&server_adr, sizeof(server_adr)))) {
        perror("bind");
        goto error;
    }
    ...
    while(1) {
        ...
        for(i=0; i<event_cnt; i++) {
            if(ep_events[i].data.fd == serverSock) {
                if((acceptedSock = accept(serverSock, (struct sockaddr*)&server_adr, &adr_size)) < 0) {
                    perror("accept");
                    ret = -1;
                    goto error;
                }
                ...
                printf("[SERVER] Connected to %s\n", inet_ntoa(*(struct in_addr *)&server_adr.sin_addr));
            }
            else {
...
launch_clients(int num_client)
{
    ...
    for(i=0; i<num_client; i++)
        pthread_create(&t_id, NULL,(void *)launch_chat, (void*)&sock);
